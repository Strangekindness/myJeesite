	...
	public void saveUser(User user) {
		if (StringUtils.isBlank(user.getId())){
			...
			if (oldUser.getOffice() != null && oldUser.getOffice().getId() != null){
				...
				CacheUtils.remove(UserUtils.USER_CACHE, UserUtils.USER_CACHE_ID_ + oldUser.getPhone());
			}
			// 更新用户数据
			user.preUpdate();
			userDao.update(user);
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	 ...
	 * @return
	 */
	public List<User> findListByUserType(String userType) {
		User user = new User();
	...
	public List<User> getListByUserIds(String userIds) {
		...
		List<User> userList = userDao.findListByUserIds(paramMap);
		return userList;
	}
}
