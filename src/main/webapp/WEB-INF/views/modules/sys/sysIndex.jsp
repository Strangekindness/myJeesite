...
<head>
    ...
    <script type="text/javascript" src="${ctxStatic}/tip/tip.js"></script>
	<style type="text/css">
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			//待审核订单数量
			var unauditedSalesOrderCount = 0;
	    	//挂起订单数量
			var suspensionSalesOrderCount = 0;
			//待审安装工单数量
			//var unauditedTicketCount = 0;
	    	//待审服务工单数量
			//var unauditedServiceTicketCount = 0;
	    	
	    	//获取订单数量
			$.ajax({
			    url:"${ctx}/crm/salesOrder/getSalesOrderCount",
			    type:"post",
			    dataType:"json",
			    success:function(data){
			    	unauditedSalesOrderCount = data.unauditedSalesOrderCount;
			    	suspensionSalesOrderCount = data.suspensionAfterTwoDayCount;
			    	showTip(unauditedSalesOrderCount, suspensionSalesOrderCount);
			    }
	     	});
		...
		//显示提示框
		function showTip(unauditedSalesOrderCount, suspensionSalesOrderCount){
			//声音提示
			var audio = new Audio();
			audio.src = "${ctxStatic}/tip/notify.wav";
			audio.play(); 
			//显示提示框
			$.messager.anim('normal', 1000);
			
			if(unauditedSalesOrderCount > 0 && suspensionSalesOrderCount == 0){
		    	//提示框持续显示15秒
				$.messager.show(0, '<a target="mainFrame" href="${ctx}/crm/salesOrder/unauditedList"><b>您有'+ unauditedSalesOrderCount +'条待审订单</b></a>', 15000);
		    }
		    
		    if(unauditedSalesOrderCount == 0 && suspensionSalesOrderCount > 0){
		    	//提示框持续显示15秒
				$.messager.show(0, '<a target="mainFrame" href="${ctx}/crm/salesOrder/suspensionAfterTwoDayList"><b>您有'+ suspensionSalesOrderCount +'条挂起订单</b></a>', 15000);
		    }
		    
		    if(unauditedSalesOrderCount > 0 && suspensionSalesOrderCount > 0){
		    	//提示框持续显示15秒
				$.messager.show(0, '<a target="mainFrame" href="${ctx}/crm/salesOrder/unauditedList"><b>您有'+ unauditedSalesOrderCount +'条待审订单</b></a><br><a target="mainFrame" href="${ctx}/crm/salesOrder/suspensionAfterTwoDayList"><b>您有'+ suspensionSalesOrderCount +'条挂起订单</b>', 15000);
		    }
		  	//提示框持续显示15秒
		    //$.messager.show(0, message, 15000);
		}
	...
	<div id="main">
	    ...
	    <div class="container-fluid">
		    ...
		    <div id="footer" class="row-fluid">
	            Copyright &copy; 2017-${fns:getConfig('copyrightYear')} ${fns:getConfig('productName')} - Powered By <a href="${fns:getConfig('author.website')}" target="_blank">${fns:getConfig('author')}</a> ${fns:getConfig('version')}
			</div>
		</div>
	</div>
	<script type="text/javascript"> 
